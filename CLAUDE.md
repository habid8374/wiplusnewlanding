...
- CMS: Sanity (Studio embebido en `/studio`). Esquemas en `sanity/schemas/`.
...
- Formularios: route handlers en `app/api/*`, Zod (`lib/schemas/forms.ts`, compartido cliente/servidor),
  Resend, Turnstile + honeypot, rate limit por IP (`lib/rate-limit.ts`).
...
8. Next 16: `proxy.ts` (no `middleware.ts`), `params`/`searchParams` son Promises,
